...
class ControllerModuleHokGallery extends Controller {
	...
	public function index() {
		$this->load->language('module/hok_gallery');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_extension_module->addModule('hok_gallery', $this->request->post);
			} else {
		...
		$data['entry_cover_background'] = $this->language->get('entry_cover_background');
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('module/hok_gallery', 'token=' . $this->session->data['token'], 'SSL')
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('module/hok_gallery', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('module/hok_gallery', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('module/hok_gallery', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		if (isset($this->request->post['button_background'])) {
			$data['button_background'] = $this->request->post['button_background'];
		} elseif (!empty($module_info)) {
			$data['button_background'] = $module_info['button_background'];
		} else {
		...
		if (isset($this->request->post['button_color'])) {
			...
			$data['button_color'] = $module_info['button_color'];
		} else {
			$data['button_color'] = '';
		}

		if (isset($this->request->post['cover_background'])) {
			$data['cover_background'] = $this->request->post['cover_background'];
		} elseif (!empty($module_info)) {
			$data['cover_background'] = $module_info['cover_background'];
		} else {
		...
		$this->response->setOutput($this->load->view('module/hok_gallery.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/hok_gallery')) {
		...
		if (!$this->request->post['popup_width']) {
			$this->error['popup_width'] = $this->language->get('error_popup_width');
		}

		if (!$this->request->post['popup_height']) {
			$this->error['popup_height'] = $this->language->get('error_popup_height');
		}
